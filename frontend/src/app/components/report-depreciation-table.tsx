...
interface ReportDepreciationTableProps {
  rows: ProvinceRow[];
}

export default function ReportDepreciationTable({ rows }: ReportDepreciationTableProps) {
  if (rows.length === 0) {
      ...
      <table className="min-w-full text-xs border-collapse">
        <thead>
          <tr className="bg-[#182286] text-white">
            ...
            <th rowSpan={2} className="px-3 py-2 border border-white/20 align-middle">Accumulated Depreciation</th>
            <th rowSpan={2} className="px-3 py-2 border border-white/20 align-middle">Book Value of Equipment</th>
          </tr>
          <tr className="bg-[#182286] text-white">
        ...
        <tbody>
          {rows.map((row) => (
            <tr key={row.province} className="text-center even:bg-gray-50">
              ...
              <td className="px-3 py-2 border border-gray-200 text-gray-800">{row.accumulatedDepreciation.toLocaleString()}</td>
              <td className="px-3 py-2 border border-gray-200 text-gray-800">{row.bookValueOfEquipment.toLocaleString()}</td>
            </tr>
          ))}
        </tbody>
      </table>
